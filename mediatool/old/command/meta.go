...
import (
	...
	"github.com/media-tool/mediatool/state"
...
type Meta struct {
	...
	// State read when calling `Context`. This is available after calling `Context`.
	state       state.State
	stateResult *StateResult

	// This can be set by the command itself to provide extra hooks.
	extraHooks []mediatool.Hook
...
func (m *Meta) Context(copts contextOpts) (*mediatool.Context, bool, error) {
	opts := m.contextOpts()

	// First try to just read the plan directly from the path given.
	f, err := os.Open(copts.Path)
	if err == nil {
		plan, err := mediatool.ReadPlan(f)
		f.Close()
		if err == nil {
			// Setup our state
...
const (
	...
	// "0", causes mediatool commands to behave as if the `-input=false` flag was
	// specified.
	InputModeEnvVar = "TF_INPUT"
)

// InputMode returns the type of input we should ask for in the form of
// mediatool.InputMode which is passed directly to Context.Input.
func (m *Meta) InputMode() mediatool.InputMode {
	if test || !m.input {
		return 0
	}
	...
	var mode mediatool.InputMode
	mode |= mediatool.InputModeProvider
	if len(m.variables) == 0 {
		mode |= mediatool.InputModeVar
		mode |= mediatool.InputModeVarUnset
	}
...
func (m *Meta) UIInput() mediatool.UIInput {
	return &UIInput{
		Colorize: m.Colorize(),
	}
}
...
func (m *Meta) PersistState(s *mediatool.State) error {
...
func (m *Meta) contextOpts() *mediatool.ContextOpts {
	var opts mediatool.ContextOpts = *m.ContextOpts
	opts.Hooks = make(
		[]mediatool.Hook,
